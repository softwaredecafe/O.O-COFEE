...
const CloudCarousel = ({
    ...
    useEffect(() => {
        ...
        loadedItems.forEach(item => {
            item.element.onload = () => {
                item.orgWidth = item.element.width;
                item.orgHeight = item.element.height;
                loadedCount++;
    ...
    const rotate = useCallback((direction) => {
        ...
        const newFrontIndex = (frontIndex - direction + images.length) % images.length;
        setFrontIndex(newFrontIndex);
        const newDestRotation = destRotation + (Math.PI / images.length) * (2 * direction);
    ...
    }, [options.autoRotate, options.autoRotateDelay, rotate, isLoaded, isMouseOver.current]);
        ...
        >
            <div className="carousel-wrapper">
                {items.map((item, i) => {
                    ...
                    const y = (sinVal * options.yRadius) * scale -200;
                    ...
                    return (
                        <div key={item.id} className="carousel-item" style={{...itemStyle, position: 'absolute'}}>
                            <img
                                ref={item.ref}
                                src={item.src}
                                alt={item.alt}
            ...
            <button className="nav-button prev" onClick={() => rotate(1)}>‹</button>
            <button className="nav-button next" onClick={() => rotate(-1)}>›</button>
        </div>
    );
};
